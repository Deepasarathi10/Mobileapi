...
#     collection = get_rmclosing_collection()
...
# async def create_dispatch(dispatch: RmClosingPost):
#     # Convert request to dict
#     new_dispatch_data = dispatch.dict()

#     # Always set current datetime in ISO format
...
#     # Insert into MongoDB
#     result = get_rmclosing_collection().insert_one(new_dispatch_data)
...
#     # Fetch from MongoDB
#     dispatch_entries = list(get_rmclosing_collection().find(query))
...
#     """
#     dispatch = get_rmclosing_collection().find_one({"_id": ObjectId(request_id)})
#     if dispatch:
...
# async def update_dispatch(request_id: str, dispatch: RmClosingPost):
#     """
...
#     result = get_rmclosing_collection().update_one({"_id": ObjectId(request_id)}, {"$set": updated_dispatch})
#     if result.modified_count == 0:
#         raise HTTPException(status_code=404, detail="Dispatch not found")
#     return {"message": "Dispatch updated successfully"}

# @router.patch("/{request_id}")
# async def patch_dispatch(request_id: str, dispatch_patch: RmClosingPost):
#     """
...
#     """
#     existing_dispatch = get_rmclosing_collection().find_one({"_id": ObjectId(request_id)})
...
#     if updated_fields:
#         result = get_rmclosing_collection().update_one({"_id": ObjectId(request_id)}, {"$set": updated_fields})
#         if result.modified_count == 0:
#             raise HTTPException(status_code=500, detail="Failed to update Dispatch")

#     updated_dispatch = get_rmclosing_collection().find_one({"_id": ObjectId(request_id)})
...
#     :return: Success message.
#     """
#     result = get_rmclosing_collection().delete_one({"_id": ObjectId(request_id)})
