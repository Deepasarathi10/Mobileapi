...
from .utils import  get_invoice_collection, get_toApprove_collection
import logging
...
async def fetch_sales_orders(query: dict) -> List[dict]:
    """
    Fetches sales orders from MongoDB asynchronously based on the query.
    """
    try:
        # Get the async collection
        collection = get_toApprove_collection()

        # Execute the query asynchronously
        logger.debug(f"Executing MongoDB query: {query}")
        results = await collection.find(query).to_list(length=None)  # Convert cursor to list asynchronously
...
async def get_sales_orders(
    ...
    try:
        ...
        # Parse date filters for delivery
        ...
        # Apply approval-related filters directly to the main query
        if approvalStatus:
            query["approvalStatus"] = {"$regex": f"^{approvalStatus}$", "$options": "i"}
            logger.debug(f"Updated query with approvalStatus filter: {query}")

        if approvalType:
            query["approvalType"] = {"$regex": f"^{approvalType}$", "$options": "i"}
            logger.debug(f"Updated query with approvalType filter: {query}")

        if summary:
            query["summary"] = {"$regex": f"^{summary}$", "$options": "i"}
            logger.debug(f"Updated query with summary filter: {query}")
        ...
        if approvalStartDate and approvalEndDate:
            ...
            end_date = end_date.replace(hour=23, minute=59, second=59)
            query["approvalDate"] = {"$gte": start_date, "$lte": end_date}
            logger.debug(f"Updated query with approvalDate range filter: {query}")
        elif approvalStartDate:
            start_date = datetime.strptime(approvalStartDate, "%d-%m-%Y")
            query["approvalDate"] = {"$gte": start_date}
            logger.debug(f"Updated query with approvalStartDate filter: {query}")
        elif approvalEndDate:
            end_date = datetime.strptime(approvalEndDate, "%d-%m-%Y")
            end_date = end_date.replace(hour=23, minute=59, second=59)
            query["approvalDate"] = {"$lte": end_date}
            logger.debug(f"Updated query with approvalEndDate filter: {query}")

        if approvalDate:
            approval_date = datetime.strptime(approvalDate, "%d-%m-%Y")
            query["approvalDate"] = approval_date
            logger.debug(f"Updated query with specific approvalDate filter: {query}")
        ...
        # Fetch sales orders from MongoDB
         # Fetch sales orders asynchronously
        raw_orders = await fetch_sales_orders(query)  # ✅ Await is mandatory
...
async def get_salesOrder_by_id(salesOrder_id: str):
    salesOrder = get_toApprove_collection().find_one({"_id": ObjectId(salesOrder_id)})
    if salesOrder:
...
async def patch_salesOrder(salesOrder_id: str, salesOrder_patch: SalesOrderPost):
    existing_salesOrder = get_toApprove_collection().find_one({"_id": ObjectId(salesOrder_id)})
    ...
    if updated_fields:
        result = get_toApprove_collection().update_one({"_id": ObjectId(salesOrder_id)}, {"$set": updated_fields})
        if result.modified_count == 0:
            raise HTTPException(status_code=500, detail="Failed to update salesOrder")

    updated_salesOrder = get_toApprove_collection().find_one({"_id": ObjectId(salesOrder_id)})
...
async def patch_salesOrder_by_saleOrderNo(saleOrderNo: str, salesOrder_patch: SalesOrderPost):
    existing_salesOrder = get_toApprove_collection().find_one({"saleOrderNo": saleOrderNo})
    if not existing_salesOrder:
        raise HTTPException(status_code=404, detail="salesOrder not found")

    updated_fields = {key: value for key, value in salesOrder_patch.dict(exclude_unset=True).items() if value is not None}
    if updated_fields:
        result = get_toApprove_collection().update_one({"saleOrderNo": saleOrderNo}, {"$set": updated_fields})
        if result.modified_count == 0:
            raise HTTPException(status_code=500, detail="Failed to update salesOrder")

    updated_salesOrder = get_toApprove_collection().find_one({"saleOrderNo": saleOrderNo})
    updated_salesOrder["_id"] = str(updated_salesOrder["_id"])
    return updated_salesOrder

@router.delete("/{salesOrder_id}")
async def delete_salesOrder(salesOrder_id: str):
    result = get_toApprove_collection().delete_one({"_id": ObjectId(salesOrder_id)})
...
async def filter_by_order_date_today():
    ...
    salesOrders = list(get_toApprove_collection().find(query))
...
def create_invoice_from_sales_order(salesOrder_id: str):
    try:
        ...
        sales_order_collection = get_toApprove_collection()
...
async def patch_multiple_credit_sales_orders_pre_invoiced(salesOrder_ids: List[str]):
        ...
        try:
            ...
            for salesOrder_id in salesOrder_ids:
                ...
                existing_sales_order = get_toApprove_collection().find_one({"_id": object_id})
                ...
                result = get_toApprove_collection().update_one({"_id": object_id}, update_data)
                ...
                updated_sales_order = get_toApprove_collection().find_one({"_id": object_id})
...
async def patch_salesOrder(salesOrder_id: str, salesOrder_patch: SalesOrderPost):
    ...
    if updated_fields:
        result = get_toApprove_collection().update_one(
            {"_id": ObjectId(salesOrder_id)},
            {"$set": updated_fields}
        )
        if result.modified_count == 0:
