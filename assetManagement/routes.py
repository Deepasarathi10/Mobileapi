...
from .utils import get_asset_collection, convert_to_string_or_none, get_counter_collection
...
async def create_asset(asset: assetPost):
    try:
        ...
        if await get_asset_collection().count_documents({}) == 0:
            await reset_counter()
        ...
        # Insert into MongoDB
        result = await get_asset_collection().insert_one(new_asset_data)
...
async def get_all_asset():
    try:
        cursor = get_asset_collection().find()
        itemasset = await cursor.to_list(length=None)  # async to_list
...
async def get_asset_by_id(asset_id: str):
    try:
        asset_data = await get_asset_collection().find_one({"_id": ObjectId(asset_id)})
        if asset_data:
...
async def update_asset(asset_id: str, asset: assetPost):
    try:
        updated_asset = asset.dict(exclude_unset=True)
        result = await get_asset_collection().update_one(
            {"_id": ObjectId(asset_id)}, {"$set": updated_asset}
        )
...
async def patch_asset(asset_id: str, asset_patch: assetPost):
    try:
        existing_asset = await get_asset_collection().find_one({"_id": ObjectId(asset_id)})
        if not existing_asset:
            raise HTTPException(status_code=404, detail="asset not found")

        updated_fields = {
            key: value for key, value in asset_patch.dict(exclude_unset=True).items() if value is not None
        }
        if updated_fields:
            result = await get_asset_collection().update_one(
                {"_id": ObjectId(asset_id)}, {"$set": updated_fields}
            )
            if result.modified_count == 0:
                raise HTTPException(status_code=500, detail="Failed to update asset")

        updated_asset = await get_asset_collection().find_one({"_id": ObjectId(asset_id)})
        updated_asset["assetId"] = str(updated_asset["_id"])
...
async def delete_asset(asset_id: str):
    try:
        result = await get_asset_collection().delete_one({"_id": ObjectId(asset_id)})
